...
def find_new_functions (module=None):
	...
	modulelist.close()
	pathlist = utils.scanPath(root='../relpy/',pattern='*.py')

	for filename in pathlist:
		modulename = 'relpy.'+filename.split('/')[2].split('.')[0]
		...
		if (strline in strmodulelist) and flag: 
			...
			for idx_line, line in enumerate(modulefilelst):
				if line.replace(' ','').replace('\t','')[:1] in ['#','']:
					pass
				else: 
					if line[:3]=='def':
						func_name = line.split(' ')[1].split('(')[0]
						if (func_name not in fun_list) or (not find_docstring(modulefilelst,idx_line)):
							...
							for next_line in modulefilelst[idx+1:]:
								if next_line.replace(' ','').replace('\t','')[:1] in ['#','']:
									...
									if 'def ' in next_line[:10]:
										if find_docstring(modulefilelst,modulefilelst.index(next_line)):
...
def find_docstring(modulefilelst,ix):
    
    line = ''
    i = ix
    while line=='':
        i+=1
        line = modulefilelst[i].replace('\n','').replace('\t','').replace(' ','')
        if line[:3] in ["'''",'"""']:
            return True
        
    print "At line", ix,":"    
    print "Function without docstring -> won't be included in doc"
    return True 
...
def main():
	...
	mod_filter = ['relpy.statBox','relpy.plotBox','relpy.utils','relpy.queryBox','relpy.connBox','relpy.fuse','relpy.tsBox'] 
	...
	if arg=='clean':
		...
		if "sphinxcontrib-fulltoc" not in listpack:
			...
			while i<5:
				...
				ans = raw_input()
				if ans=='y':
					os.system('pip install sphinxcontrib-fulltoc') 
					i=5
				elif ans=='n' or i==4:
					raise SystemExit('Aborting build...\n'+'Building HTML failed')
				else: 
